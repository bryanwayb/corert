...
struct gc_flavor_debug_contract
{
	const uint16_t major_version = 1; // breaking changes
	const uint16_t minor_version = 0; // back-compatible changes
#ifdef MULTIPLE_HEAPS
	// Fill this out when its time to support server gc
#else
	generation** const generation_table_addr = (generation**)&generation_table;
	uint8_t** const alloc_allocated_addr = &gc_heap::alloc_allocated;
	heap_segment** const ephemeral_heap_segment_addr = &gc_heap::ephemeral_heap_segment;
	const uint32_t number_generations = NUMBERGENERATIONS;
	const uint32_t offset_of_generation_allocation_context = offsetof(generation, allocation_context);
	const uint32_t offset_of_generation_allocation_start = offsetof(generation, allocation_start);
	const uint32_t offset_of_generation_start_segment = offsetof(generation, start_segment);
	const uint32_t size_of_generation = sizeof(generation);
	const uint32_t offset_of_heap_segment_mem = offsetof(heap_segment, mem);
	const uint32_t offset_of_heap_segment_allocated = offsetof(heap_segment, allocated);
	const uint32_t offset_of_heap_segment_next = offsetof(heap_segment, next);
	const uint32_t offset_of_alloc_context_alloc_ptr = offsetof(alloc_context, alloc_ptr);
	const uint32_t offset_of_alloc_context_alloc_limit = offsetof(alloc_context, alloc_limit);
	const uint32_t min_object_size = MIN_OBJECT_SIZE;
#endif
};
